...
export function EnhancedChatbot() {
  ...
  const handleSend = async () => {
    ...
    try {
      // Prepare the messages array format expected by the API
      ...
      // Make an API request to your chatbot backend
      ...
      if (responseText && responseText.includes("[DEADLINE]")) {
        parseAndAddDeadline(responseText)
      }
    } catch (error) {
  ...
  // Parse deadline information from chatbot response
  const parseAndAddDeadline = async (message: string) => {
    try {
      ...
      if (deadlineMatch && deadlineMatch[1]) {
        ...
        // Send the deadline data to the API to be saved
    ...
    <Card className="h-[400px] flex flex-col">
      ...
      <CardContent className="flex-1 flex flex-col">
        ...
        <div className="flex gap-2 mt-4">
          <Input
            placeholder="Ask about courses, requirements, or deadlines..."
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
